...
filepath = os.path.join(script_dir, "0_아/p_1_1.wav")

# TensorFlow Metal 비활성화 및 버전 체크 우회 (import 전에 설정)
...
def install_missing_packages_from_requirements():
    """requirements.txt를 사용해 누락된 패키지 자동 설치"""
    try:
        # TensorFlow 버전 체크 문제 해결을 위한 임시 패치
        import tensorflow as tf
        # 유효한 버전 형식으로 설정
        if not hasattr(tf, '__version__') or not tf.__version__:
        ...
        print(f"📦 누락된 패키지 발견: {e}")
        print("requirements.txt에서 자동 설치합니다...")
...
try:
    import tensorflow as tf
    # TensorFlow 버전 속성이 없는 경우 처리
    if not hasattr(tf, '__version__') or not tf.__version__:
        tf.__version__ = "2.13.0"
    
    # tensorflow.compat이 없는 경우 패치
...
try:
    import tensorflow_hub as hub
    print("✅ TensorFlow Hub import 완료")
except ImportError as e:
    print(f"❌ TensorFlow Hub import 실패: {e}")
...
try:
    ...
    print("⚠️ 경고를 무시하고 계속 진행합니다...")

print("✅ 모든 라이브러리 import 시도 완료")
...
def setup_tensorflow():
    ...
    try:
        ...
        if hasattr(tf, 'config'):
            ...
            print("⚠️ TensorFlow config 없음 - 기본 설정 사용")
    except Exception as e:
        print(f"⚠️ TensorFlow 설정 실패: {e} - 계속 진행")
...
def estimate_pitch_spice_only(audio, sr=16000):
    ...
    try:
        ...
        device_context = tf.device('/CPU:0') if hasattr(tf, 'device') else None
        ...
        if device_context:
            with device_context:
                # 모델 워밍업
                dummy_audio = tf.zeros([1000], dtype=tf.float32) if hasattr(tf, 'zeros') else tf.constant(np.zeros(1000), dtype=tf.float32)
                try:
            ...
            # device 없이 실행
            dummy_audio = tf.constant(np.zeros(1000), dtype=tf.float32)
            try:
                _ = model.signatures["serving_default"](dummy_audio)
                print("모델 워밍업 완료")
            except Exception as warmup_error:
