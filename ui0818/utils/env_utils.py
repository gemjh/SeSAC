...
def activate_conda_environment():
    ...
    try:
        ...
        if 'CLAP_PC' not in current_python:
            ...
            if WINOS:
                sesac_python = os.path.join(conda_base, "envs", "CLAP_PC", "python.exe")
            else:
            ...
            # streamlit run 명령어 형태로 재실행
            env = os.environ.copy()
            env['SKIP_ENV_CHECK'] = '1'
            
            # streamlit run 형태인지 확인
            if len(sys.argv) >= 2 and 'streamlit' in sys.argv[0]:
                # streamlit run app.py 형태
                app_file = sys.argv[1] if len(sys.argv) > 1 else 'app.py'
                subprocess.run([sesac_python, "-m", "streamlit", "run", app_file] + sys.argv[2:], env=env)
            else:
                # python app.py 형태
                import __main__
                main_file = getattr(__main__, '__file__', sys.argv[0])
                subprocess.run([sesac_python, main_file] + sys.argv[1:], env=env)
            sys.exit(0)
        else:
