                ...
                if d2result != "available":
                    ...
                    with st.spinner('분석 중... (약 10-30초 소요)'):
                        try:
                            ...
                            script_path = os.path.join(current_dir, 'models', 'talk_clean', 'direct_d2result.py')
                            python_path = "/Users/kimberlyjojohirn/opt/anaconda3/envs/clap/bin/python"
                            ...
                            if result.returncode == 0:
                                # predict_script.py는 마지막 줄에 숫자만 출력
                                try:
                                    lines = output.split('\n')
                                    score = int(lines[-1])
                                    ...
                                    if output.startswith("SUCCESS:"):
                                        score = output.split("SUCCESS:")[1]
                                        st.session_state[result_key] = {"type": "success", "message": f"예상 점수: {score}점"}
                                    else:
                                        st.session_state[result_key] = {"type": "error", "message": f"결과 파싱 실패: {output}"}
                            else:
                                error_msg = result.stderr if result.stderr else output
                                st.session_state[result_key] = {"type": "error", "message": f"분석 실패: {error_msg}"}
                                
                        except subprocess.TimeoutExpired:
                            st.session_state[result_key] = {"type": "error", "message": "분석 시간이 초과되었습니다."}
                        except Exception as e:
