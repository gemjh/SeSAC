...
def get_assess_lst(patient_id, assess_type=None):
    ...
    try:
        ...
        sql = ""
        sql += "select distinct lst.ORDER_NUM, lst.PATIENT_ID, p.name, lst.AGE, p.SEX, flst.ASSESS_TYPE, lst.ASSESS_DATE, lst.REQUEST_ORG, lst.ASSESS_PERSON "
        ...
        ret_df = pd.DataFrame(rows, columns=['ORDER_NUM', 'PATIENT_ID', 'NAME', 'AGE', 'SEX', 'ASSESS_TYPE', 'ASSESS_DATE', 'REQUEST_ORG', 'ASSESS_PERSON'])
        msg = f'{len(ret_df)}건의 데이터가 조회되었습니다.'
        return msg, ret_df
...
def get_patient_info(patient_id, order_num):
    ...
    try:
        ...
        ret_df = pd.DataFrame(rows, columns=['PATIENT_ID', 'NAME', 'SEX', 'REQUEST_ORG', 'ASSESS_DATE', 'ASSESS_PERSON', 'AGE', 'EDU', 'POST_STROKE_DATE', 'DIAGNOSIS', 'DIAGNOSIS_ETC', 'STROKE_TYPE', 'LESION_LOCATION', 'HEMIPLEGIA', 'HEMINEGLECT', 'VISUAL_FIELD_DEFECT'])
        msg = f'{len(ret_df)}건의 데이터가 조회되었습니다.'
        return msg, ret_df
