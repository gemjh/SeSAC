...
def zip_upload():
    ...
    if btn_apply & (patient_id is not None) & (uploaded_file is not None):
        ...
        with zipfile.ZipFile(uploaded_file, "r") as zip_ref:
                                ...
                                if os.path.isdir(path_clap_a_item) & (clap_A_cd.get(clap_a_item) != None):
                                    ...
                                    for item in clap_a_sub_lst:
                                        if item.startswith('p_'):
                                            ...
                                            sql += "('"+patient_id+"', "+str(order_num)+", 'CLAP_A', '"+clap_A_cd.get(clap_a_item)+"', "+item.split('_')[1]+", "+item.split('_')[2][0]+", '"+patient_id+"', 'clap_a/"+clap_a_item+"', '"+item+"', "+f"{duration:.2f}"+", "+str(rate)+"),\n"
                                        else:
                                            continue
                            sql = sql[:-2]
                            #print(sql)
                                    ...
                                    for item in clap_d_sub_lst:
                                        if item.startswith('p_'):
                                            ...
                                            with wave.open(os.path.join(path_clap_d_item, item), 'rb') as wav_file:
                                                ...
                                                duration = frames / float(rate)        # 총 시간 (초)
                                                #print(f"{item} Duration: {duration:.2f} seconds, {rate}")
                                            sql += "('"+patient_id+"', "+str(order_num)+", 'CLAP_D', '"+clap_D_cd.get(clap_d_item)+"', "+item.split('_')[1]+", "+item.split('_')[2][0]+", '"+patient_id+"', 'clap_d/"+clap_d_item+"', '"+item+"', "+f"{duration:.2f}"+", "+str(rate)+"),\n"
                                        else:
                                            continue
                            sql = sql[:-2]
                            #print(sql)
            ...
            cursor.close()        
            conn.close()
            st.session_state.patients_info=True