        ...
        for category in categories:
            ...
            if talk_obj is not None:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
                    ...
                    st.session_state.pa_result = rang.talk_obj(file_path)
...
def show_clap_d_detail():
    ...
    if not clap_d_data.empty:
        ...
        print("파일 업로드 시작")
        print("------------------------\n\n\n")
        talk_clean_folder = st.file_uploader("또박또박 말하기", type=['wav', 'mp3'], key="talk_clean", 
        label_visibility="collapsed")
        print("파일 업로드 완료")
        print("------------------------\n\n\n")
        if talk_clean_folder is not None:
            print("if문 진입")
            print("------------------------\n\n\n")
            #from models.talk_clean import CLAP_D_2 as hj
            from models.talk_clean.CLAP_D_2 import d2result
            # 세션 반복 사용 때문에 뮤텍스 걸리는걸수도 있음
            talk_clean = d2result("ui/p_1_0.wav")
            # with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
            #     print("with 입장")
            #     print("------------------------\n\n\n")
            #     from models.talk_clean.CLAP_D_2 import d2result
            #     print("모델 입장")
            #     print("------------------------\n\n\n")
            #     tmp_file.write(talk_clean.getvalue())
            #     file_path = tmp_file.name
            #     print("hj 함수 진입")
            #     print("------------------------\n\n\n")
            #     st.session_state.talk_clean = d2result(file_path)
            #     print("hj 함수 탈출")
            #     print("------------------------\n\n\n")
            #     print(st.session_state.talk_clean)
            #     st.rerun()
