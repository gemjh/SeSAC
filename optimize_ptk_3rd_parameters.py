...
def optimize_ptk_parameters():
    ...
    height_values=np.arange(.15,.2,.05)
    distance_values = np.arange(2000, 3200, 100)  # 2000 ~ 4800, 200 간격 (원본 3000 주변) # distance_values는 2000~3200으로 고정하는게 좋을듯
    ...
    # 결과 저장
    results_df.to_csv('H:/ptk_3rd_optimization_results.csv', index=False)
    print(f"💾 결과 저장 완료: H:/ptk_3rd_optimization_results.csv")
...
def visualize_ptk_optimization_results(results_df):
    ...
    plt.tight_layout()
    plt.savefig('H:/ptk_3rd_optimization_visualization.png', dpi=150, bbox_inches='tight')
    plt.show()
    
    print("💾 시각화 결과 저장 완료:")
    print("  - H:/ptk_3rd_optimization_visualization.png")
...
def analyze_ptk_parameter_sensitivity(results_df):
    ...
    plt.tight_layout()
    plt.savefig('H:/ptk_3rd_parameter_sensitivity_analysis.png', dpi=150, bbox_inches='tight')
    plt.show()
...
def create_excel_comparison(results_df, best_params):
    ...
    # Excel 파일로 저장
    output_path = 'H:/ptk_3rd_comparison.xlsx'
    ...
    try:
        ...
        # CSV로 대체 저장
        comparison_df.to_csv('H:/ptk_3rd_comparison.csv', index=False)
...
if __name__ == "__main__":
    ...
    if result is None or result[0] is None:
        ...
        # 최종 보고서 저장
        with open('H:/ptk_3rd_optimization_report.txt', 'w', encoding='utf-8') as f:
        ...
        print("\n💾 최종 보고서 저장 완료: H:/ptk_3rd_optimization_report.txt")
