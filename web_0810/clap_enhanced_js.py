...
def show_clap_d_detail():
    ...
    # --------------------------- 왜 안되는지 모르겠음 주석처리하면 잘됨 ---------------------------
    ah_input=st.file_uploader("음성파일(아)")
    with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
        tmp_file.write(ah_input.getvalue())
        file_path = tmp_file.name
        a_sound=ah.analyze_pitch_stability(file_path)
    # --------------------------- 왜 안되는지 모르겠음 주석처리하면 잘됨 ---------------------------
    if not a_sound:
        a_sound=100

    pa_sound, ta_sound, ka_sound, ptk_sound=st.file_uploader("음성파일(퍼)"),st.file_uploader("음성파일(터)"),st.file_uploader("음성파일(커)"),st.file_uploader("음성파일(퍼터커)")
    if all([pa_sound, ta_sound, ka_sound, ptk_sound]):
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
            tmp_file.write(pa_sound.getvalue())
            file_path = tmp_file.name
            pa_repeat=ptk.count_peaks_from_waveform(file_path)
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
            tmp_file.write(ta_sound.getvalue())
            file_path = tmp_file.name
            ta_repeat=ptk.count_peaks_from_waveform(file_path)
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
            tmp_file.write(ka_sound.getvalue())
            file_path = tmp_file.name
            ka_repeat=ptk.count_peaks_from_waveform(file_path)
        with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp_file:
            tmp_file.write(ptk_sound.getvalue())
            file_path = tmp_file.name
            ptk_repeat=ptk.count_peaks_from_waveform(file_path)
